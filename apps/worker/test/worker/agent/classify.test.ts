...
import { classifyMessage, CLASSIFICATION_LABELS } from "../../../src/modules/agent/classify";

type Run = (model: string, inputs: unknown) => Promise<unknown>;

describe("classifyMessage", () => {
  it("filters labels to the candidate set", async () => {
    const run = vi.fn().mockResolvedValue({ response: JSON.stringify({ labels: ["work", "nope"], confidence: 0.8 }) });
  ...
  it("clamps confidence and tolerates invalid payloads", async () => {
    const run = vi.fn().mockResolvedValue({ response: JSON.stringify({ labels: "not-array", confidence: 9 }) });
