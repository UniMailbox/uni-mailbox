...
function ctx(messages: { subject: string; text_body: string }[] = [{ subject: "s", text_body: "b" }]) {
...
describe("summarizeThreadTool", () => {
  ...
  it("returns a summary payload and calls summarizeThread", async () => {
    const spy = vi.spyOn(summarizeMod, "summarizeThread").mockResolvedValue("summary");
    const tool = summarizeThreadTool(ctx() as never);
    const result = await tool.handler({ thread_id: "t1" }, { sessionId: null, requestId: "req" });
    expect(spy).toHaveBeenCalled();
    expect(result.structuredContent).toMatchObject({ summary: "summary", model: "8b" });
  });
