...
import {
  ...
  type AdminResourceKey, type PermissionKey,
  type EndpointRequest, type EndpointResponse,
} from "@unimailbox/contracts";
import { apiClient } from "../../lib/api/index";

type ManagedResource = Extract<AdminResourceKey, "users" | "roles" | "domains" | "provider-connections">;
type WritableResource = ManagedResource | "signatures" | "settings" | "webhook-events" | "provider-sync";
...
const providerCreateBody = AdminCreateSchema.options[3].omit({ resource: true });
...
export const adminFormSchemas = {
  createUser: userCreateBody.extend({ roleIds: commaSeparated(userCreateBody.shape.roleIds) }),
  createRole: roleCreateBody.extend({ permissions: commaSeparated(roleCreateBody.shape.permissions) }),
  createDomain: domainCreateBody,
  createProviderConnection: providerCreateBody,
  manageUser: z.discriminatedUnion("action", [
    z.object({ action: z.literal("delete"), id: userUpdate.shape.id, displayName: z.string(), status: z.string(), roleIds: z.string() }),
    z.object({ action: z.literal("update"), id: userUpdate.shape.id, displayName: z.union([z.literal(""), userUpdate.shape.displayName.unwrap()]), status: z.union([z.literal(""), userUpdate.shape.status.unwrap()]), roleIds: z.union([z.literal(""), commaSeparated(userUpdate.shape.roleIds.unwrap())]) }),
  ]),
  manageRole: z.discriminatedUnion("action", [
    z.object({ action: z.literal("delete"), id: roleUpdate.shape.id, description: z.string(), permissions: z.string() }),
    z.object({ action: z.literal("update"), id: roleUpdate.shape.id, description: roleUpdate.shape.description, permissions: commaSeparated(roleUpdate.shape.permissions) }),
  ]),
  manageDomain: z.discriminatedUnion("action", [
    z.object({ action: z.literal("delete"), id: domainUpdate.shape.id, status: z.string(), outboundConnectionId: z.string() }),
    z.object({ action: z.literal("update"), id: domainUpdate.shape.id, status: z.union([z.literal(""), domainUpdate.shape.status.unwrap()]), outboundConnectionId: z.union([z.literal(""), domainUpdate.shape.outboundConnectionId.unwrap().unwrap()]) }),
  ]),
  manageProviderConnection: z.object({ id: providerUpdate.shape.id, status: z.union([z.literal(""), providerUpdate.shape.status.unwrap()]), apiKey: z.union([z.literal(""), providerUpdate.shape.apiKey.unwrap()]), webhookSecret: z.union([z.literal(""), providerUpdate.shape.webhookSecret.unwrap()]) }),
...
const WRITE_PERMISSION: Record<WritableResource, PermissionKey> = {
  users: "user.manage", roles: "role.manage", domains: "domain.manage",
  "provider-connections": "domain.manage", signatures: "signature.manage",
  settings: "settings.manage", "webhook-events": "webhook_event.delete",
  "provider-sync": "provider.sync",
};

export function canAdminWrite(resource: WritableResource, permissions: readonly PermissionKey[]): boolean {
...
export const adminKeys = {
  ...
  detail: (resource: ManagedResource | "webhook-events", id: string) => [...adminKeys.resource(resource), id.trim()] as const,
...
export function adminQueryOptions(resource: AdminResourceKey, auditSearch = "") {
  const normalizedSearch = auditSearch.trim();
  const queryKey = resource === "audit-events" ? adminKeys.auditEvents(normalizedSearch) : adminKeys.resource(resource);
  const queryFn = async (): Promise<AdminQueryData> => {
    switch (resource) {
      case "users": return apiClient.request(administrationEndpoints.users, {});
      case "roles": return apiClient.request(administrationEndpoints.roles, {});
      case "domains":
      case "signatures": return apiClient.request(administrationEndpoints.domains, {});
      case "settings": return apiClient.request(administrationEndpoints.settings, {});
      case "provider-connections": return apiClient.request(administrationEndpoints.providerConnections, {});
      case "webhook-events": return apiClient.request(administrationEndpoints.webhookEvents, { query: {} });
      case "audit-events": return apiClient.request(administrationEndpoints.auditEvents, { query: normalizedSearch ? { q: normalizedSearch } : {} });
      case "analytics": return apiClient.request(administrationEndpoints.analytics, {});
    }
  };
